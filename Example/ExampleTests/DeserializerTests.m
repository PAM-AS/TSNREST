...
- (void)setUp {
    ...
    [[TSNRESTManager sharedManager] addObjectMap:[TSNRESTObjectMap autogeneratedMapForClass:[Image class]]];
...
- (void)testThatDeserializingMultipleSimultaniouslyWorks {
    ...
    [self waitForExpectationsWithTimeout:1 handler:^(NSError *error) {
        ...
        XCTAssert([Brand MR_countOfEntities] == expectedBrandCount, @"Got wrong amount of brands (%li - %@) in our db, expected %li, parsing failed.", [Brand MR_countOfEntities] , [[Brand MR_findAll] valueForKey:@"systemId"], expectedBrandCount);
...
- (void)testThatDeserializingManyRelationsWork {
    NSURL *productUrl = [[NSURL alloc] initFileURLWithPath:[[NSBundle mainBundle] pathForResource:@"Fixtures/JSON/products" ofType:@"json"]];
    NSURLRequest *productRequest = [NSURLRequest requestWithURL:productUrl];
    ...
    [self waitForExpectationsWithTimeout:1 handler:^(NSError *error) {
        NSUInteger expectedImageCount = 4;
        XCTAssert([Image MR_countOfEntities] == expectedImageCount, @"Got wrong amount of images (%li) in our db, expected %li, parsing failed.", [Image MR_countOfEntities], expectedImageCount);
...
- (void)testThatDeserializerParsesDates {
    NSURL *imageUrl = [[NSURL alloc] initFileURLWithPath:[[NSBundle mainBundle] pathForResource:@"Fixtures/JSON/images" ofType:@"json"]];
    NSURLRequest *imageRequest = [NSURLRequest requestWithURL:imageUrl];
    
    XCTestExpectation *imageDatataskExpectation = [self expectationWithDescription:@"Image datatask complete"];
    
    NSURLSessionDataTask *imageTask = [NSURLSessionDataTask dataTaskWithRequest:imageRequest success:^(NSData *data, NSURLResponse *response, NSError *error) {
        XCTAssert(data.length > 0, @"No valid data was provided from the fixtures.");
    } failure:^(NSData *data, NSURLResponse *response, NSError *error, NSInteger statusCode) {
        XCTFail(@"Data task failed.");
    } finally:^(NSData *data, NSURLResponse *response, NSError *error) {
        [imageDatataskExpectation fulfill];
    } parseResult:YES];
    [imageTask resume];
    
    [self waitForExpectationsWithTimeout:1 handler:^(NSError *error) {
        NSArray *images = [Image MR_findAll];
        XCTAssert(images.count > 0, @"No images was loaded from fixtures");
        for (Image *image in images) {
            XCTAssertNotNil(image.createdAt, @"Image was loaded with invalid date");
        }
    }];
}
